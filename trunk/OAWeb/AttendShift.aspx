...
<body>
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
								...
								<table cellspacing=0 borderColorDark='<%=Application[Session["Style"].ToString()+"xtable_bordercolordark"]%>' 
									...
									<tr>
										<td style="background-color:<%=Application[Session["Style"].ToString()+"xtable_titlebgcolor"]%>">                      
                                            <asp:Label ID="Label1" runat="server" Text="Label" SkinID="Title">轮班设置</asp:Label></td>
									</tr>
								</table>
							</td>
						</tr>
						...
						<tr>
							<td style="background-color:<%=Application[Session["Style"].ToString()+"xtable_bgcolor"]%>">	
                                        <asp:GridView ID="GridView1" runat="server" DataSourceID="SqlDataSource1" OnRowCommand="GridView1_RowCommand">
                                            ...
                                            <Columns>
                                                <asp:HyperLinkField DataNavigateUrlFields="AutoID" Text="&lt;img src=images/button_edit.gif border=0 alt=修改 /&gt;" DataNavigateUrlFormatString="AttendShiftAdd.aspx?AutoID={0}" HeaderText="编辑" />
                                                <asp:BoundField DataField="ShiftName" HeaderText="轮班名称" SortExpression="ShiftName" />
                                                <asp:BoundField DataField="ShiftType" HeaderText="轮班方式" SortExpression="ShiftType" />
      ...
      ,[EnterpriseID]
      ,[ShiftName]
      ,case [ShiftType] when 1 then '周轮班' when 2 then '月轮班' else '状态不明' end  as [ShiftType]
      ,[ShiftYear]
      ,[ShiftMonth]
      ,[Shift1]
      ,[Shift2]
